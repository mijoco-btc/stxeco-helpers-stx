...
export async function getBurnHeightToRewardCycle(
  stacksApi: string,
...
export async function getRewardCycleToBurnHeight(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<any> {
...
export async function getPoxCycleInfo(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<PoxCycleInfo> {
  const totalStacked = await getTotalUstxStacked(
    stacksApi,
    poxContractId,
    cycle
  );
  const numbEntriesRewardCyclePoxList = await getNumbEntriesRewardCyclePoxList(
    stacksApi,
    poxContractId,
    cycle
  );
  const totalPoxRejection = await getTotalPoxRejection(
    stacksApi,
    poxContractId,
    cycle
  );
...
export async function getPoxCycleInfoRelative(
  stacksApi: string,
  mempoolApi: string,
  poxContractId: string,
  cycle: number,
  ...
  const result1 = await getRewardCycleToBurnHeight(
    stacksApi,
    poxContractId,
    cycle
  );
  ...
  try {
    const firstBitcoinBlock = await fetchBlockAtHeight(
      mempoolApi,
      firstBlockHeight
    );
  ...
  try {
    const lastBitcoinBlock = await fetchBlockAtHeight(
      mempoolApi,
      lastBlockHeight
    );
...
export async function getTotalUstxStacked(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<any> {
...
export async function getRewardSetPoxAddress(
  stacksApi: string,
  poxContractId: string,
  cycle: number,
  index: number
...
export async function getNumbEntriesRewardCyclePoxList(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<any> {
...
export async function getRewardSetSize(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<any> {
...
export async function getTotalPoxRejection(
  stacksApi: string,
  poxContractId: string,
  cycle: number
): Promise<any> {
...
export async function getAllowanceContractCallers(
  stacksApi: string,
  poxContractId: string,
  address: string,
  contract: string,
...
export async function getPartialStackedByCycle(
  stacksApi: string,
  network: string,
  poxContractId: string,
  address: string,
  ...
  try {
    const functionArgs = [
      ...
      sender.indexOf(".") === -1
        ...
        : `0x${serializeCV(
            contractPrincipalCV(sender.split(".")[0], sender.split(".")[1])
          )}`,
    ];
    const data = {
      contractAddress: poxContractId.split(".")[0],
...
export async function getStackerInfoFromContract(
  stacksApi: string,
  network: string,
  poxContractId: string,
  address: string,
  ...
  return {
    ...
    poxRejection:
      cycle > 0
        ? await getPoxRejection(stacksApi, poxContractId, address, cycle)
        : undefined,
  };
}
  ...
  try {
    ...
    return result.value
      ? {
          ...
          bitcoinAddr: getAddressFromHashBytes(
            network,
            result.value.value["pox-addr"].value.hashbytes.value,
            result.value.value["pox-addr"].value.version.value
          ),
        }
      : undefined;
...
export async function getPoxRejection(
  stacksApi: string,
  poxContractId: string,
  address: string,
  cycle: number
...
export async function checkCallerAllowed(
  stacksApi: string,
  poxContractId: string,
  stxAddress: string,
  tip?: number | undefined
...
export async function verifySignerKeySig(
  stacksApi: string,
  network: string,
  poxContractId: string,
  auth: VerifySignerKey,
  tip?: number | undefined
): Promise<Stacker | undefined> {
...
export async function readDelegationEvents(
  stacksApi: string,
  network: string,
  poxContractId: string,
  ...
  try {
    ...
    if (val) {
      for (const event of val.results) {
        const cycle = await getBurnHeightToRewardCycle(
          stacksApi,
          poxContractId,
          event.block_height
        );
